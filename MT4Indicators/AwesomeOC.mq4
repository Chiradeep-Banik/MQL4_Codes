...
//--- drawing settings
   ...
   SetIndexStyle(1,DRAW_HISTOGRAM,STYLE_SOLID,4,Bisque);
   SetIndexStyle(2,DRAW_HISTOGRAM,STYLE_SOLID,4,Tomato);
...
//--- macd
   for(i=0; i<limit; i++)
      ExtAOBuffer[i]=(iMA(NULL,0,PERIOD_FAST,0,MODE_SMA,PRICE_MEDIAN,i)+
                      iMA(NULL,0,PERIOD_FAST,0,MODE_SMA,PRICE_MEDIAN,i+1))/2-
                     (iMA(NULL,0,PERIOD_SLOW,0,MODE_SMA,PRICE_MEDIAN,i)+
                      iMA(NULL,0,PERIOD_SLOW,0,MODE_SMA,PRICE_MEDIAN,i+1))/2;
//      ExtAOBuffer[i]=(iOpen(NULL,0,i+1)+iHigh(NULL,0,i+1)+iLow(NULL,0,i+1)+iClose(NULL,0,i+1)+iOpen(NULL,0,i)+iHigh(NULL,0,i)+iLow(NULL,0,i)+iClose(NULL,0,i))/8;
