...
extern bool      SendMailOn = false; // Line added.  Default is true.
...
extern int       BackSteps=0;/*Used to be extern int now just int. Leave at 0*/
extern int       ShowingSteps=1;/*Used to be extern int now just int.  Leave at 1*/
extern bool      FractalAsTD=false;/*Used to be extern bool now just bool.  Leave at false, otherwise Trend Lines based on Fractal Points not TD Points*/
...
string TakeProfitLowTD(int L1,int L2,int Step,int Col)/*Draw Sell TD Price Projection(s)*/
{
 ...
 ExtMapBuffer6[0] = NormalizeDouble(k-LC1,Digits) ; 
...
string TDMain(int Step)
{
 ...
 double Col[20];Col[0]=UpperProjectionLineColour/*Original Col[0]=Red, Colour for Current Upper TD Projection*/;Col[2]=Magenta;Col[4]=Chocolate;Col[6]=Goldenrod;Col[8]=SlateBlue;
   ...
   H1=GetHighTD(Step);
   H2=GetNextHighTD(H1);
//   Print("HInd=", H1, " High1= ", High[H1], " High2= ", High[H2], " TP=", DoubleToStr(HTP/Point,0));
   L1=GetLowTD(Step);
   L2=GetNextLowTD(L1);
//   Print("LInd=",L1, " Low1= ", Low[L1], " Low2= ", Low[L2], " TP=", DoubleToStr(LTP/Point,0));
//   TrendLineBreakUp=High[H2]-(High[H2]-High[H1])/(H2-H1)*H2;//added line
//   TrendLineBreakDown=Low[L2]+(Low[L1]-Low[L2])/(L2-L1)*L2;//added line
   TrendLineBreakUp=High[H1];
   TrendLineBreakDown=Low[L1];
...
//--------------------------------------------------------------------
   if(AlertsOn)//added this Alerts section
   {
     ...
     if(Open[0]>TrendLineBreakUp && TrendLineBreakUpFlag==False)
     {
     //Print("Upper TrendLine Break ",Symbol()," ",Period()," ",Bid);
     Alert("UTL Break>",TrendLineBreakUp," on ",Symbol()," ",Period()," @ ",Bid); 
     if(SendMailOn)//added this Alerts section
     {
      SendMail("UTL Break>" + TrendLineBreakUp + " on " + Symbol() + " " + Period() + " @ " + Bid, "");
     }
     TrendLineBreakUpFlag=True;
     }   
   if(Open[0]<TrendLineBreakDown && TrendLineBreakDownFlag==False)
     {
     //Print("Lower Trendline Break ",Symbol()," ",Period()," ",Bid);
     Alert("LTL Break<",TrendLineBreakDown," on ",Symbol()," ",Period()," @ ",Bid); 
     if(SendMailOn)//added this Alerts section
     {
      SendMail("LTL Break<" + TrendLineBreakDown + " on " + Symbol() + " " + Period() + " @ " + Bid, "");
     }
