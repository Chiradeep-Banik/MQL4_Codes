...
extern int from  = 0;
extern int maP  = 50;
extern bool ÀËÌËË = false;
extern bool —ÚÂÎÍË = false;
extern bool “Ó˜Í‡ = true;
extern bool UseSound=true;
extern bool AlertPossible    = true;
extern bool AlertOnlySound   = true;
extern bool SoundBuy  = true;
extern bool SoundSell = true;
extern string Sound          = "alert2.wav";
extern bool SendMailPossible = false;
extern int —»√Õ¿À‹Õ€…_¡¿– = 1;
...
//+------------------------------------------------------------------+
 static int PrevSignal = 0, PrevTime = 0;
    ...
    if(ÀËÌËË == true){
	 ...
	 if( SoundBuy==true){      
        if( LineMinBuffer[—»√Õ¿À‹Õ€…_¡¿– +1]<LineMidBuffer[—»√Õ¿À‹Õ€…_¡¿– +1] && LineMinBuffer[—»√Õ¿À‹Õ€…_¡¿–]>LineMidBuffer[—»√Õ¿À‹Õ€…_¡¿–] ){
        if (AlertOnlySound) {PlaySound(Sound);} else {            
            Alert("aNina_v1 (", Symbol(), ", ", Period(), ")  -  BUY!!!");PlaySound(Sound);
            if (SendMailPossible) SendMail(Symbol()+ " M"+ Period()+ " aNina_v1  -  BUY!!!", "");
          }}}      
    if(  SoundSell == true){      
        if((  LineMinBuffer[—»√Õ¿À‹Õ€…_¡¿– +1]>LineMidBuffer[—»√Õ¿À‹Õ€…_¡¿– +1] && LineMinBuffer[—»√Õ¿À‹Õ€…_¡¿–]<LineMidBuffer[—»√Õ¿À‹Õ€…_¡¿–] ) ){
        if (AlertOnlySound) {PlaySound(Sound);} else {           
            Alert("aNina_v1 (", Symbol(), ", ", Period(), ")  -  SELL!!!");PlaySound(Sound);
            if (SendMailPossible) SendMail(Symbol()+ " M"+ Period()+ " aNina_v1   -  SELL!!!", "");
          }}}}}                  
//+------------------------------------------------------------------+ 
  if(—ÚÂÎÍË == true){
   //+------------------------------------------------------------------+         
        if (LineBuyBuffer[—»√Õ¿À‹Õ€…_¡¿–] != EMPTY_VALUE && LineBuyBuffer[—»√Õ¿À‹Õ€…_¡¿–] != 0 && SoundBuy)
         {
         SoundBuy = False;
            if (UseSound) PlaySound (Sound);
               if(AlertOnlySound){         
               Alert("aNina_v1 (", Symbol(), ", ", Period(), ")  -  BUY!!!");                             
               if (SendMailPossible) SendMail(Symbol()+ " M"+ Period()+ " aNina_v1 -  BUY!", ""); 
            }              
         } 
      if (!SoundBuy && (LineBuyBuffer[—»√Õ¿À‹Õ€…_¡¿–] == EMPTY_VALUE || LineBuyBuffer[—»√Õ¿À‹Õ€…_¡¿–] == 0)) SoundBuy = True;  
            
  
     if (LineSellBuffer[—»√Õ¿À‹Õ€…_¡¿–] != EMPTY_VALUE && LineSellBuffer[—»√Õ¿À‹Õ€…_¡¿–] != 0 && SoundSell)
         {
         SoundSell = False;
            if (UseSound) PlaySound (Sound); 
             if(AlertOnlySound){                    
             Alert("aNina_v1 (", Symbol(), ", ", Period(), ")  -  SELL!!!");             
             if (SendMailPossible) SendMail(Symbol()+ " M"+ Period()+ " aNina_v1 -  SELL!!!", ""); 
             }            
         } 
      if (!SoundSell && (LineSellBuffer[—»√Õ¿À‹Õ€…_¡¿–] == EMPTY_VALUE || LineSellBuffer[—»√Õ¿À‹Õ€…_¡¿–] == 0)) SoundSell = True; 
      
       //+------------------------------------------------------------------+  
  }
   if(“Ó˜Í‡ == true){
  //+------------------------------------------------------------------+         
        if (LineExitBuffer[—»√Õ¿À‹Õ€…_¡¿–] != EMPTY_VALUE && LineExitBuffer[—»√Õ¿À‹Õ€…_¡¿–] != 0 && SoundBuy)
         {
         SoundBuy = False;
            if (UseSound) PlaySound (Sound);
               if(AlertOnlySound){         
               Alert("aNina_v1 (", Symbol(), ", ", Period(), ")  -  œÓˇ‚ËÎ‡Ò¸ ÚÓ˜Í‡!!!");                             
               if (SendMailPossible) SendMail(Symbol()+ " M"+ Period()+ " aNina_v1 -  œÓˇ‚ËÎ‡Ò¸ ÚÓ˜Í‡!", ""); 
            }              
         } 
      if (!SoundBuy && (LineExitBuffer[—»√Õ¿À‹Õ€…_¡¿–] == EMPTY_VALUE || LineExitBuffer[—»√Õ¿À‹Õ€…_¡¿–] == 0)) SoundBuy = True;  
